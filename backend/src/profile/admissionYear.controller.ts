...
export class AdmissionYearController {
  constructor(private readonly admissionYearService: AdmissionYearService) {}

  @UseGuards(JwtAuthGuard)
  @Post()
  async create(
    @Body() createAdmissionYearDto: CreateAdmissionYearDto,
  ...
  @Get(':id')
  async findOneById(
    @Param('id') admissionYearId: number,
  ): Promise<AdmissionYearEntity> {
  ...
  async update(
    @Param('id') admissionYearId: number,
    @Body() updateAdmissionYearDto: UpdateAdmissionYearDto,
  ): Promise<AdmissionYearEntity> {
    return await this.admissionYearService.updateOne(
      admissionYearId,
